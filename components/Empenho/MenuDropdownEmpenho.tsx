...
import EditEmpenho from "./EditEmpenho";
...
const MenuDropdownEmpenho = () => {
  ...
  const deleteId = async (e: any) => {
    ...
    if (deleted.deleted) {
      router.push(`/despesa/${nrDespesaRouter}`);
    } else {
      openModal();
    }
  };
  ...
  const editId = async (e: any) => {
    e.preventDefault();
    if (empenhoId == idEmpenhoRouter) {
      setEmpenhoId(idEmpenhoRouter + " ");
    } else {
      setEmpenhoId(typeof idEmpenhoRouter === "string" ? idEmpenhoRouter : "");
    }
  };
  return (
    <>
      ...
      <EditEmpenho empenhoId={empenhoId} />
    </>
  );
};

export default MenuDropdownEmpenho;
