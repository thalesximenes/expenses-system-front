...
const MenuDropdownDespesa = () => {
  ...
  const deleteId = async (e: any) => {
    e.preventDefault();
    setDespesaId("");
    const response = await fetch(`${DESPESA_API_BASE_URL}/${nrDespesaRouter}`, {
      method: "DELETE",
    });
  ...
  const editId = async (e: any) => {
    e.preventDefault();
    if (despesaId == nrDespesaRouter) {
      setDespesaId(nrDespesaRouter + " ");
    } else {
      setDespesaId(typeof nrDespesaRouter === "string" ? nrDespesaRouter : "");
    }
  };
  ...
  return (
    <>
      ...
      <EditDespesa despesaId={despesaId} />
    </>
  );
};

export default MenuDropdownDespesa;
